...
public class rtkcmn{
    /* constants -----------------------------------------------------------------*/
    public static final long POLYCRC32  = 0xEDB88320; /* CRC32 polynomial */
    public static final long POLYCRC24Q = 0x1864CFB;  /* CRC24Q polynomial */
    public static double SQR(double x){
        return Math.pow(x, 2);
    }
    public static final double MAX_VAR_EPH = SQR(300.0);  /* max variance eph to reject satellite (m^2) */

    public static final double gpst0[]={1980,1, 6,0,0,0}; /* gps time reference */
    public static final double gst0 []={1999,8,22,0,0,0}; /* galileo system time reference */
    public static final double bdt0 []={2006,1, 1,0,0,0}; /* beidou time reference */

    public static double[][] leaps = new double[][]{ /* leap seconds (y,m,d,h,m,s,utc-gpst) */
        {2017,1,1,0,0,0,-18},
        {2015,7,1,0,0,0,-17},
        {2012,7,1,0,0,0,-16},
        {2009,1,1,0,0,0,-15},
...
final double[] chisqr = new double[]{      /* chi-sqr(n) (alpha=0.001) */
...
final double[] lam_carr = new double{ /* carrier wave length (m) */
        rtklib.CLIGHT/rtklib.FREQ1,rtklib.CLIGHT/rtklib.FREQ2,rtklib.CLIGHT/rtklib.FREQ5,rtklib.CLIGHT/rtklib.FREQ6,rtklib.CLIGHT/rtklib.FREQ7,
        rtklib.CLIGHT/rtklib.FREQ8,rtklib.CLIGHT/rtklib.FREQ9
    };
final rtklib.prcopt_t prcopt_default= new rtklib.prcopt_t{ /* defaults processing options */
        rtklib.PMODE_SINGLE,0,2,rtklib.SYS_GPS,   /* mode,soltype,nf,navsys */
            15.0*rtklib.D2R,{{0,0}},           /* elmin,snrmask */
...
final rtklib.solopt_t solopt_default={ /* defaults solution output options */
        rtklib.SOLF_LLH,rtklib.TIMES_GPST,1,3,    /* posf,times,timef,timeu */
            0,1,0,0,0,0,0,              /* degf,outhead,outopt,outvel,datum,height,geoid */
            0,0,0,                      /* solstatic,sstat,trace */
            {0.0,0.0},                  /* nmeaintv */
            " ",""                      /* separator/program name */
    };
final String[] formatstrs = new String[]{    /* stream format strings */
        "RTCM 2",                   /*  0 */
        "RTCM 3",                   /*  1 */
        "NovAtel OEM6",             /*  2 */
        "NovAtel OEM3",             /*  3 */
    ...
    static String[] obscodes = new String[]{       /* observation code strings */
    ...
    static String[][] codepris = new String[][]{  /* code priority table */
    ...
    public class fatalfunc_t{
        fatalfunc=null; /* fatal callback function */
    }

    /* crc tables generated by util/gencrc ---------------------------------------*/
    static final int tbl_CRC16[]={
    ...
    /* fatal error ---------------------------------------------------------------*/
    public static void fatalerr(final char[] format, ...)
    {
     ...
     *-----------------------------------------------------------------------------*/
    public static void add_fatal(fatalfunc_t func)
    {
        fatalfunc=func;
    }
    ...
    public static int satno(int sys, int prn)
    {
        ...
        switch (sys) {
            case rtklib.SYS_GPS:
                if (prn<rtklib.MINPRNGPS||rtklib.MAXPRNGPS<prn) return 0;
                return prn-rtklib.MINPRNGPS+1;
            case rtklib.SYS_GLO:
                if (prn<rtklib.MINPRNGLO||rtklib.MAXPRNGLO<prn) return 0;
                return rtklib.NSATGPS+prn-rtklib.MINPRNGLO+1;
            case rtklib.SYS_GAL:
                if (prn<rtklib.MINPRNGAL||rtklib.MAXPRNGAL<prn) return 0;
                return rtklib.NSATGPS+rtklib.NSATGLO+prn-rtklib.MINPRNGAL+1;
            case rtklib.SYS_QZS:
                if (prn<rtklib.MINPRNQZS||rtklib.MAXPRNQZS<prn) return 0;
                return rtklib.NSATGPS+rtklib.NSATGLO+rtklib.NSATGAL+prn-rtklib.MINPRNQZS+1;
            case rtklib.SYS_CMP:
                if (prn<rtklib.MINPRNCMP||rtklib.MAXPRNCMP<prn) return 0;
                return rtklib.NSATGPS+rtklib.NSATGLO+rtklib.NSATGAL+rtklib.NSATQZS+prn-rtklib.MINPRNCMP+1;
            case rtklib.SYS_IRN:
                if (prn<rtklib.MINPRNIRN||rtklib.MAXPRNIRN<prn) return 0;
                return rtklib.NSATGPS+rtklib.NSATGLO+rtklib.NSATGAL+rtklib.NSATQZS+rtklib.NSATCMP+prn-rtklib.MINPRNIRN+1;
            case rtklib.SYS_LEO:
                if (prn<rtklib.MINPRNLEO||rtklib.MAXPRNLEO<prn) return 0;
                return rtklib.NSATGPS+rtklib.NSATGLO+rtklib.NSATGAL+rtklib.NSATQZS+rtklib.NSATCMP+rtklib.NSATIRN+
                        prn-rtklib.MINPRNLEO+1;
            case rtklib.SYS_SBS:
                if (prn<rtklib.MINPRNSBS||rtklib.MAXPRNSBS<prn) return 0;
                return rtklib.NSATGPS+rtklib.NSATGLO+rtklib.NSATGAL+rtklib.NSATQZS+rtklib.NSATCMP+rtklib.NSATIRN+rtklib.NSATLEO+
                        prn-rtklib.MINPRNSBS+1;
        }
        return 0;
    }
    ...
    {
        int sys=rtklib.SYS_NONE;
        if (sat<=0||rtklib.MAXSAT<sat) sat=0;
        else if (sat<=rtklib.NSATGPS) {
            sys=rtklib.SYS_GPS; sat+=rtklib.MINPRNGPS-1;
        }
        else if ((sat-=rtklib.NSATGPS)<=rtklib.NSATGLO) {
            sys=rtklib.SYS_GLO; sat+=rtklib.MINPRNGLO-1;
        }
        else if ((sat-=rtklib.NSATGLO)<=rtklib.NSATGAL) {
            sys=rtklib.SYS_GAL; sat+=rtklib.MINPRNGAL-1;
        }
        else if ((sat-=rtklib.NSATGAL)<=rtklib.NSATQZS) {
            sys=rtklib.SYS_QZS; sat+=rtklib.MINPRNQZS-1;
        }
        else if ((sat-=rtklib.NSATQZS)<=rtklib.NSATCMP) {
            sys=rtklib.SYS_CMP; sat+=rtklib.MINPRNCMP-1;
        }
        else if ((sat-=rtklib.NSATCMP)<=rtklib.NSATIRN) {
            sys=rtklib.SYS_IRN; sat+=rtklib.MINPRNIRN-1;
        }
        else if ((sat-=rtklib.NSATIRN)<=rtklib.NSATLEO) {
            sys=rtklib.SYS_LEO; sat+=rtklib.MINPRNLEO-1;
        }
        else if ((sat-=rtklib.NSATLEO)<=rtklib.NSATSBS) {
            sys=rtklib.SYS_SBS; sat+=rtklib.MINPRNSBS-1;
        }
        else sat=0;
        if (prn != 0 ) prn=sat;
        return sys;
     ...
     *-----------------------------------------------------------------------------*/
    public static int satid2no(final String id)
    {
        int sys,prn;
        char code;

        if (sscanf(id,"%d",&prn)==1) {
            if      (rtklib.MINPRNGPS<=prn&&prn<=rtklib.MAXPRNGPS) sys=rtklib.SYS_GPS;
            else if (rtklib.MINPRNSBS<=prn&&prn<=rtklib.MAXPRNSBS) sys=rtklib.SYS_SBS;
            else if (rtklib.MINPRNQZS<=prn&&prn<=rtklib.MAXPRNQZS) sys=rtklib.SYS_QZS;
            else return 0;
        ...
        switch (code) {
            case 'G': sys=rtklib.SYS_GPS; prn+=rtklib.MINPRNGPS-1; break;
            case 'R': sys=rtklib.SYS_GLO; prn+=rtklib.MINPRNGLO-1; break;
            case 'E': sys=rtklib.SYS_GAL; prn+=rtklib.MINPRNGAL-1; break;
            case 'J': sys=rtklib.SYS_QZS; prn+=rtklib.MINPRNQZS-1; break;
            case 'C': sys=rtklib.SYS_CMP; prn+=rtklib.MINPRNCMP-1; break;
            case 'I': sys=rtklib.SYS_IRN; prn+=rtklib.MINPRNIRN-1; break;
            case 'L': sys=rtklib.SYS_LEO; prn+=rtklib.MINPRNLEO-1; break;
            case 'S': sys=rtklib.SYS_SBS; prn+=100; break;
            default: return 0;
        }
        return satno(sys,prn);
    }
     ...
     * return : none
     *-----------------------------------------------------------------------------*/
    public static void satno2id(int sat, String id)
    {
        Integer prn = new Integer(0);
        switch (satsys(sat, prn)) {
        case rtklib.SYS_GPS: String.format(id,"G%02d",prn-rtklib.MINPRNGPS+1); return;
        case rtklib.SYS_GLO: String.format(id,"R%02d",prn-rtklib.MINPRNGLO+1); return;
        case rtklib.SYS_GAL: String.format(id,"E%02d",prn-rtklib.MINPRNGAL+1); return;
        case rtklib.SYS_QZS: String.format(id,"J%02d",prn-rtklib.MINPRNQZS+1); return;
        case rtklib.SYS_CMP: String.format(id,"C%02d",prn-rtklib.MINPRNCMP+1); return;
        case rtklib.SYS_IRN: String.format(id,"I%02d",prn-rtklib.MINPRNIRN+1); return;
        case rtklib.SYS_LEO: String.format(id,"L%02d",prn-rtklib.MINPRNLEO+1); return;
        case rtklib.SYS_SBS: String.format(id,"%03d" ,prn); return;
        }
        id = "";
    }
    ...
    public static int satexclude(int sat, double var, int svh, final rtklib.prcopt_t opt)
    {
        ...
        if (sys==rtklib.SYS_QZS) svh&=0xFE; /* mask QZSS LEX health */
        if (svh==1) {
     ...
     *-----------------------------------------------------------------------------*/
    public static int testsnr(int base, int freq, double el, double snr,
                   final rtklib.snrmask_t mask)
    {
        double minsnr,a;
        int i;

        if (!mask.ena[base]||freq<0||freq>=rtklib.NFREQ) return 0;

        a=(el*rtklib.R2D+5.0)/10.0;
        i=(int)Math.floor(a); a-=i;
     ...
     *-----------------------------------------------------------------------------*/
    public static int obs2code(final String obs, Integer freq)
    {
        int i;
        if (freq == null) {
            freq=0;
...
* notes  : obs codes are based on reference [6] and qzss extension
*-----------------------------------------------------------------------------*/
    public static String code2obs(int code, Integer freq)
    {
        if (freq == null) freq=0;
        if (code<=rtklib.CODE_NONE||rtklib.MAXCODE<code) return "";
        if (freq) freq=obsfreqs[code];
        return obscodes[code];
    }
    /* set code priority -----------------------------------------------------------
     * set code priority for multiple codes in a frequency
     * args   : int    sys     I     system (or of rtklib.SYS_???)
     ...
     * return : none
     *-----------------------------------------------------------------------------*/
    public static void setcodepri(int sys, int freq, final String pri)
    {
        trace(3,"setcodepri:sys=%d freq=%d pri=%s\n",sys,freq,pri);

        if (freq<=0||rtklib.MAXFREQ<freq) return;
        if (sys == rtklib.SYS_GPS) codepris[0][freq-1] = pri;
        if (sys == rtklib.SYS_GLO) codepris[1][freq-1] = pri;
        if (sys == rtklib.SYS_GAL) codepris[2][freq-1] = pri;
        if (sys == rtklib.SYS_QZS) codepris[3][freq-1] = pri;
        if (sys == rtklib.SYS_SBS) codepris[4][freq-1] = pri;
        if (sys == rtklib.SYS_CMP) codepris[5][freq-1] = pri;
        if (sys == rtklib.SYS_IRN) codepris[6][freq-1] = pri;
    }
    /* get code priority -----------------------------------------------------------
    ...
    public static int getcodepri(int sys, short code, final String opt)
    {
        ...
        switch (sys) {
            case rtklib.SYS_GPS: i=0; optstr="-GL%2s"; break;
            case rtklib.SYS_GLO: i=1; optstr="-RL%2s"; break;
            case rtklib.SYS_GAL: i=2; optstr="-EL%2s"; break;
            case rtklib.SYS_QZS: i=3; optstr="-JL%2s"; break;
            case rtklib.SYS_SBS: i=4; optstr="-SL%2s"; break;
            case rtklib.SYS_CMP: i=5; optstr="-CL%2s"; break;
            case rtklib.SYS_IRN: i=6; optstr="-IL%2s"; break;
            default: return 0;
        }
        obs=code2obs(code,j);
     ...
     *-----------------------------------------------------------------------------*/
    public static long getbitu(final short[] buff, int pos, int len)
    {
        long bits=0;
        int i;
        for (i=pos;i<pos+len;i++) bits=(bits<<1)+((buff[i/8]>>(7-i%8))&1);
        return bits;
    }
    public static int getbits(final short[] buff, int pos, int len)
    {
        long bits=getbitu(buff,pos,len);
        if (len<=0||32<=len||!(bits&(1<<(len-1)))) return (int)bits;
        return (int)(bits|(~0<<len)); /* extend sign */
    }
     ...
     * return : none
     *-----------------------------------------------------------------------------*/
    public static void setbitu(unsigned char *buff, int pos, int len, unsigned int data)
    {
    ...
    public static void setbits(unsigned char *buff, int pos, int len, int data)
    {
     ...
     *-----------------------------------------------------------------------------*/
    public static long rtk_crc32(final short[] buff, int len)
    {
        long int crc=0;
        int i,j;
     ...
     *-----------------------------------------------------------------------------*/
    public static long rtk_crc24q(final short[] buff, int len)
    {
        long crc=0;
        int i;
     ...
     *-----------------------------------------------------------------------------*/
    public static int rtk_crc16(final short[] buff, int len)
    {
        int crc=0;
        int i;
     ...
     *-----------------------------------------------------------------------------*/
    public static int decode_word(long word, short[] data)
    {
        final long hamming[]={
    ...
    *t=epoch2time(ep);
    ...
    static int read_leaps_text(File fp)
    {
        ...
        while (fgets(buff,sizeof(buff),fp)&&n<rtklib.MAXLEAPS) {
    ...
    *day=epoch2time(ep);
     ...
     *-----------------------------------------------------------------------------*/
    public static String time_str(rtklib.gtime_t t, int n)
    {
        static char buff[64];
        time2str(t,buff,n);
        return buff;
...
#define Rx(t,X) do { \
    ...
    *deps*=1E-4*AS2R;
    ...
    final double ep2000[]={2000,1,1,12,0,0};
        ...
        /* iau 1976 precession */
        ze=(2306.2181*t+0.30188*t2+0.017998*t3)*rtklib.AS2R;
        th=(2004.3109*t-0.42665*t2-0.041833*t3)*rtklib.AS2R;
        z =(2306.2181*t+1.09468*t2+0.018203*t3)*rtklib.AS2R;
        eps=(84381.448-46.8150*t-0.00059*t2+0.001813*t3)*rtklib.AS2R;
        Rz(-z,R1); Ry(th,rtklib.R2); Rz(-ze,rtklib.R3);
    ...
    static void addpcv(final rtklib.pcv_t pcv, rtklib.pcvs_t pcvs)
    {
        rtklib.pcv_t pcvs_pcv = new rtklib.pcv_t();
     ...
     *-----------------------------------------------------------------------------*/
    public static int readpcv(final String file, rtklib.pcvs_t *pcvs)
    {
        pcv_t *pcv;
        char *ext;
        int i,stat;
     ...
     * return : none
     *-----------------------------------------------------------------------------*/
    public static void readpos(final char *file, final char *rcv, double *pos)
    {
     ...
     * return : status (1:ok,0:error)
     *-----------------------------------------------------------------------------*/
    public static int geterp(final rtklib.erp_t erp, rtklib.gtime_t time, double[] erpv)
    {
    final double ep[]={2000,1,1,12,0,0};
        double mjd,day,a;
        int i,j,k;
        ...
        if (mjd<=erp.data.get(0).mjd) {
            day=mjd-erp.data.get(0).mjd;
            erpv[0]=erp.data.get(0).xp     +erp.data.get(0).xpr*day;
            erpv[1]=erp.data.get(0).yp     +erp.data.get(0).ypr*day;
            erpv[2]=erp.data.get(0).ut1_utc-erp.data.get(0).lod*day;
            erpv[3]=erp.data.get(0).lod;
            return 1;
        }
        if (mjd>=erp.data.get(erp.n-1).mjd) {
            day=mjd-erp.data.get(erp.n-1).mjd;
            erpv[0]=erp.data.get(erp.n-1).xp     +erp.data.get(erp.n-1).xpr*day;
            erpv[1]=erp.data.get(erp.n-1).yp     +erp.data.get(erp.n-1).ypr*day;
            erpv[2]=erp.data.get(erp.n-1).ut1_utc-erp.data.get(erp.n-1).lod*day;
            erpv[3]=erp.data.get(erp.n-1).lod;
            return 1;
        }
        for (j=0,k=erp.n-1;j<k-1;) {
            i=(j+k)/2;
            if (mjd<erp.data.get(i).mjd) k=i; else j=i;
        }
        if (erp.data.get(j).mjd==erp.data.get(j+1).mjd) {
            a=0.5;
        }
        else {
     ...
     *-----------------------------------------------------------------------------*/
    public static void uniqnav(rtklib.nav_t nav)
    {
        int i,j;
    ...
    public static void traceopen(final char *file)
    {
        gtime_t time=utc2gpst(timeget());
        char path[1024];
    ...
    public static void trace(int level, final char *format, ...)
    {
        va_list ap;
    ...
    public static void tracet(int level, final char *format, ...)
    {
        va_list ap;
    ...
    public static void tracemat(int level, final double *A, int n, int m, int p, int q)
    {
        if (!fp_trace||level>level_trace) return;
        matfprint(A,n,m,p,q,fp_trace); fflush(fp_trace);
    }
    public static void traceobs(int level, final obsd_t *obs, int n)
    {
        char str[64],id[16];
        int i;
    ...
    public static void tracenav(int level, final nav_t *nav)
    {
    ...
    public static void tracepclk(int level, final nav_t *nav)
    {
        char s[64],id[16];
        int i,j;
    ...
    public static void traceb(int level, final unsigned char *p, int n)
    {
        int i;
        if (!fp_trace||level>level_trace) return;
     ...
     *-----------------------------------------------------------------------------*/
    public static int execcmd(final char *cmd)
    {
#ifdef WIN32
     ...
     *-----------------------------------------------------------------------------*/
    public static int expath(final char *path, char *paths[], int nmax)
    {
        int i,j,n=0;
        char tmp[1024];
#ifdef WIN32
     ...
     *-----------------------------------------------------------------------------*/
    public static void createdir(final char *path)
    {
        char buff[1024],*p;
    ...
    *p='\0';
    ...
    /* replace string ------------------------------------------------------------*/
    static int repstr(char *str, final char *pat, final char *rep)
    {
     ...
     *-----------------------------------------------------------------------------*/
    public static int reppaths(final String path, char *rpath[], int nmax, rtklib.gtime_t ts,
                               rtklib.gtime_t te, final String rov, final String base)
    {
        gtime_t time;
     ...
     *-----------------------------------------------------------------------------*/
    public static double satwavelen(int sat, int frq, final rtklib.nav_t nav)
    {
    final double freq_glo[]={rtklib.FREQ1_GLO,rtklib.FREQ2_GLO};
    final double dfrq_glo[]={rtklib.DFRQ1_GLO,rtklib.DFRQ2_GLO};
        int i,sys=satsys(sat,null);

        if (sys==rtklib.SYS_GLO) {
            if (0<=frq&&frq<=1) {
                for (i=0;i<nav.ng;i++) {
                    if (nav.geph[i].sat!=sat) continue;
                    return rtklib.CLIGHT/(freq_glo[frq]+dfrq_glo[frq]*nav.geph[i].frq);
                }
            }
            else if (frq==2) { /* L3 */
                return rtklib.CLIGHT/rtklib.FREQ3_GLO;
            }
        }
        else if (sys==rtklib.SYS_CMP) {
            if      (frq==0) return rtklib.CLIGHT/rtklib.FREQ1_CMP; /* B1 */
            else if (frq==1) return rtklib.CLIGHT/rtklib.FREQ2_CMP; /* B2 */
            else if (frq==2) return rtklib.CLIGHT/rtklib.FREQ3_CMP; /* B3 */
        }
        else {
            if      (frq==0) return rtklib.CLIGHT/rtklib.FREQ1; /* L1/E1 */
            else if (frq==1) return rtklib.CLIGHT/rtklib.FREQ2; /* L2 */
            else if (frq==2) return rtklib.CLIGHT/rtklib.FREQ5; /* L5/E5a */
            else if (frq==3) return rtklib.CLIGHT/rtklib.FREQ6; /* L6/LEX */
            else if (frq==4) return rtklib.CLIGHT/rtklib.FREQ7; /* E5b */
            else if (frq==5) return rtklib.CLIGHT/rtklib.FREQ8; /* E5a+b */
            else if (frq==6) return rtklib.CLIGHT/rtklib.FREQ9; /* S */
        }
        return 0.0;
    }
    /* geometric distance ----------------------------------------------------------
     ...
     *-----------------------------------------------------------------------------*/
    public static double geodist(final double *rs, final double *rr, double *e)
    {
        double r;
        int i;

        if (norm(rs,3)<rtklib.RE_WGS84) return -1.0;
        for (i=0;i<3;i++) e[i]=rs[i]-rr[i];
        r=norm(e,3);
        for (i=0;i<3;i++) e[i]/=r;
        return r+rtklib.OMGE*(rs[0]*rr[1]-rs[1]*rr[0])/rtklib.CLIGHT;
    }
     ...
     *-----------------------------------------------------------------------------*/
    public static double satazel(final double *pos, final double *e, double *azel)
    {
        double az=0.0,el=rtklib.PI/2.0,enu[3];

        if (pos[2]>-rtklib.RE_WGS84) {
            ecef2enu(pos,e,enu);
    ...
    public static void dops(int ns, final double *azel, double elmin, double *dop)
    {
        double H[4*rtklib.MAXSAT],Q[16],cosel,sinel;
        int i,n;

        for (i=0;i<4;i++) dop[i]=0.0;
        for (i=n=0;i<ns&&i<rtklib.MAXSAT;i++) {
     ...
     *-----------------------------------------------------------------------------*/
    public static double ionmodel(rtklib.gtime_t t, double[] ion, final double[] pos,
                       final double[] azel)
    {
    final double[] ion_default= new double[]{/* 2004/1/1 */
        ...
        psi=0.0137/(azel[1]/rtklib.PI+0.11)-0.022;

        /* subionospheric latitude/longitude (semi-circle) */
        phi=pos[0]/rtklib.PI+psi*Math.cos(azel[0]);
        if      (phi> 0.416) phi= 0.416;
        else if (phi<-0.416) phi=-0.416;
        lam=pos[1]/rtklib.PI+psi*Math.sin(azel[0])/Math.cos(phi*rtklib.PI);

        /* geomagnetic latitude (semi-circle) */
        phi+=0.064*Math.cos((lam-1.617)*rtklib.PI);
        ...
        /* slant factor */
        f=1.0+16.0*Math.pow(0.53-azel[1]/rtklib.PI,3.0);
        ...
        x=2.0*rtklib.PI*(tt-50400.0)/per;

        return rtklib.CLIGHT*f*(Math.abs(x)<1.57?5E-9+amp*(1.0+x*x*(-0.5+x*x/24.0)):5E-9);
    }
     ...
     *-----------------------------------------------------------------------------*/
    public static double ionmapf(final double[] pos, final double[] azel)
    {
        if (pos[2]>=rtklib.HION) return 1.0;
        return 1.0/Math.cos(Math.asin((rtklib.RE_WGS84+pos[2])/(rtklib.RE_WGS84+rtklib.HION)*Math.sin(rtklib.PI/2.0-azel[1])));
    }
     ...
     *-----------------------------------------------------------------------------*/
    public static double ionppp(final double[] pos, final double[] azel, double re,
                         double hion, double[] posp)
    {
        double cosaz,rp,ap,sinap,tanap;

        rp=re/(re+hion)*Math.cos(azel[1]);
        ap=rtklib.PI/2.0-azel[1]-Math.asin(rp);
        sinap=Math.sin(ap);
        tanap=Math.tan(ap);
        cosaz=Math.cos(azel[0]);
        posp[0]=Math.asin(Math.sin(pos[0])*Math.cos(ap)+Math.cos(pos[0])*sinap*cosaz);

        if ((pos[0]> 70.0*rtklib.D2R&& tanap*cosaz>Math.tan(rtklib.PI/2.0-pos[0]))||
                (pos[0]<-70.0*rtklib.D2R&&-tanap*cosaz>Math.tan(rtklib.PI/2.0+pos[0]))) {
            posp[1]=pos[1]+rtklib.PI-Math.asin(sinap*Math.sin(azel[0])/Math.cos(posp[0]));
        }
        else {
     ...
     *-----------------------------------------------------------------------------*/
    public static double tropmodel(rtklib.gtime_t time, final double[] pos, final double[] azel,
                            double humi)
    {
    ...
    static double interpc(final double coef[], double lat)
    {
        int i=(int)(lat/15.0);
    ...
    static double nmf(gtime_t time, final double pos[], final double azel[],
                      double *mapfw)
    {
        /* ref [5] table 3 */
    ...
    final double aht[]={ 2.53E-5, 5.49E-3, 1.14E-3}; /* height correction */
     ...
     *-----------------------------------------------------------------------------*/
    public static double tropmapf(gtime_t time, final double pos[], final double azel[],
                           double *mapfw)
    {
#ifdef IERS_MODEL
    final double ep[]={2000,1,1,12,0,0};
    ...
    static double interpvar(double ang, final double *var)
    {
     ...
     *-----------------------------------------------------------------------------*/
    public static void antmodel(final rtklib.pcv_t pcv, final double[] del, final double[] azel,
                         int opt, double[] dant)
    {
        double e[3],off[3],cosel=Math.cos(azel[1]);
        int i,j;
     ...
     * return : none
     *-----------------------------------------------------------------------------*/
    public static void antmodel_s(final pcv_t *pcv, double nadir, double *dant)
    {
        int i;
     ...
     * return : none
     *-----------------------------------------------------------------------------*/
    public static void sunmoonpos(gtime_t tutc, final double *erpv, double *rsun,
     ...
     * return : none
     *-----------------------------------------------------------------------------*/
    public static void csmooth(rtklib.obs_t obs, int ns)
    {
        double[][][] Ps = new double[2][rtklib.MAXSAT][NFREQ]={{{0}}},Lp[2][rtklib.MAXSAT][NFREQ]={{{0}}},dcp;
     ...
     *-----------------------------------------------------------------------------*/
    public static int rtk_uncompress(final String file, String uncfile)
    {
        int stat=0;
        String p = new String();
