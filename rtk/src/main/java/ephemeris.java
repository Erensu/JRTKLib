...
public class ephemeris {
    ...
    public static final double RE_GLO  = 6378136.0;        /* radius of earth (m)            ref [2] */
    public static final double MU_GPS  = 3.9860050E14;     /* gravitational constant         ref [1] */
    public static final double MU_GLO  = 3.9860044E14;     /* gravitational constant         ref [2] */
    public static final double MU_GAL  = 3.986004418E14;   /* earth gravitational constant   ref [7] */
    public static final double MU_CMP  = 3.986004418E14;   /* earth gravitational constant   ref [9] */
    public static final double J2_GLO  = 1.0826257E-3;     /* 2nd zonal harmonic of geopot   ref [2] */

    public static final double OMGE_GLO = 7.292115E-5;      /* earth angular velocity (rad/s) ref [2] */
    public static final double OMGE_GAL = 7.2921151467E-5;  /* earth angular velocity (rad/s) ref [7] */
    public static final double OMGE_CMP = 7.292115E-5;      /* earth angular velocity (rad/s) ref [9] */

    public static final double SIN_5 = -0.0871557427476582; /* sin(-5.0 deg) */
    public static final double COS_5 =  0.9961946980917456; /* cos(-5.0 deg) */

    public static final double ERREPH_GLO = 5.0;            /* error of glonass ephemeris (m) */
    public static final double TSTEP    = 60.0;             /* integration step glonass ephemeris (s) */
    public static final double RTOL_KEPLER = 1E-13;         /* relative tolerance for Kepler equation */
    ...
    public static final double STD_GAL_NAPA = 500.0;        /* error of galileo ephemeris for NAPA (m) */

    public static final double MAX_ITER_KEPLER = 30;        /* max number of iteration of Kelpler */
    ...
    {
    final double ura_value[]={
            2.4,3.4,4.85,6.85,9.65,13.65,24.0,48.0,96.0,192.0,384.0,768.0,1536.0,
            3072.0,6144.0
    };
        if (sys==rtklib.SYS_GAL) { /* galileo sisa (ref [7] 5.1.11) */
    ...
    void alm2pos(rtklib.gtime_t time, final rtklib.alm_t alm, double rs, double dts)
    {
        ...
        rtkcmn.trace(4,"alm2pos : time=%s sat=%2d\n",rtkcmn.time_str(time,3),alm.sat);

        tk=rtkcmn.timediff(time,alm.toa);

        if (alm.A<=0.0) {
            rs[0]=rs[1]=rs[2]=*dts=0.0;
            return;
        }
        mu=rtkcmn.satsys(alm.sat,null)==rtklib.SYS_GAL?MU_GAL:MU_GPS;

        M=alm.M0+Math.sqrt(mu/(alm.A*alm.A*alm.A))*tk;
        for (n=0,E=M,Ek=0.0;Math.abs(E-Ek)>RTOL_KEPLER&&n<MAX_ITER_KEPLER;n++) {
            Ek=E; E-=(E-alm.e*Math.sin(E)-M)/(1.0-alm.e*Math.cos(E));
        }
        if (n>=MAX_ITER_KEPLER) {
            rtkcmn.trace(2,"alm2pos: kepler iteration overflow sat=%2d\n",alm.sat);
            return;
        }
        ...
        x=r*Math.cos(u); y=r*Math.sin(u); sinO=Math.sin(O); cosO=Math.cos(O); cosi=Math.cos(i);
    ...
    *dts=alm.f0+alm.f1*tk;
     ...
     *-----------------------------------------------------------------------------*/
    static double eph2clk(rtklib.gtime_t time, final rtklib.eph_t eph)
    {
        double t;
        int i;

        rtkcmn.trace(4,"eph2clk : time=%s sat=%2d\n",rtkcmn.time_str(time,3),eph.sat);

        t=rtkcmn.timediff(time,eph.toc);
     ...
     *          (tgd or bgd)
     *-----------------------------------------------------------------------------*/
    public static void eph2pos(rtklib.gtime_t time, final rtklib.eph_t eph, Double[] rs, Double dts,
                        Double var)
    {
        double tk,M,E,Ek,sinE,cosE,u,r,i,O,sin2u,cos2u,x,y,sinO,cosO,cosi,mu,omge;
        double xg,yg,zg,sino,coso;
        int n,sys;
        Integer prn = new Integer();

        rtkcmn.trace(4,"eph2pos : time=%s sat=%2d\n",rtkcmn.time_str(time,3),eph.sat);

        if (eph.A<=0.0) {
            rs[0]=rs[1]=rs[2]=dts=var=0.0;
            return;
        }
        tk=rtkcmn.timediff(time,eph.toe);

        switch ((sys=rtkcmn.satsys(eph.sat,prn))) {
        case rtklib.SYS_GAL: mu=MU_GAL; omge=OMGE_GAL; break;
        case rtklib.SYS_CMP: mu=MU_CMP; omge=OMGE_CMP; break;
        default:      mu=MU_GPS; omge= rtklib.OMGE;     break;
    }
        ...
        if (n>=MAX_ITER_KEPLER) {
            rtkcmn.trace(2,"eph2pos: kepler iteration overflow sat=%2d\n",eph.sat);
            return;
        }
        sinE=Math.sin(E); cosE=Math.cos(E);

        rtkcmn.trace(4,"kepler: sat=%2d e=%8.5f n=%2d del=%10.3e\n",eph.sat,eph.e,n,E-Ek);
        ...
        if (sys==rtklib.SYS_CMP&&(eph.flag==2||(eph.flag==0&&prn<=5))) {
        ...
        dts-=2.0*Math.sqrt(mu*eph.A)*eph.e*sinE/SQR(rtklib.CLIGHT);
     ...
     * notes  : see ref [2]
     *-----------------------------------------------------------------------------*/
    public static double geph2clk(rtklib.gtime_t time, final rtklib.geph_t geph)
    {
        double t;
        int i;

        rtkcmn.trace(4,"geph2clk: time=%s sat=%2d\n",rtkcmn.time_str(time,3),geph.sat);

        t=rtkcmn.timediff(time,geph.toe);
     ...
     * notes  : see ref [2]
     *-----------------------------------------------------------------------------*/
    public static void geph2pos(rtklib.gtime_t time, final rtklib.geph_t geph, Double[] rs, Double dts,
                         Double var)
    {
        double t,tt;
        double x = new double[];
     ...
     * notes  : see ref [3]
     *-----------------------------------------------------------------------------*/
    public static double seph2clk(rtklib.gtime_t time, final rtklib.seph_t seph)
    {
        double t;
        int i;

        rtkcmn.trace(4,"seph2clk: time=%s sat=%2d\n",rtkcmn.time_str(time,3),seph.sat);

        t=rtkcmn.timediff(time,seph.t0);
                         ...
                         Double var)
    {
        ...
        rtkcmn.trace(4,"seph2pos: time=%s sat=%2d\n",rtkcmn.time_str(time,3),seph.sat);
    ...
    static rtklib.eph_t seleph(rtklib.gtime_t time, int sat, int iode, final rtklib.nav_t nav)
    {
        ...
        rtkcmn.trace(4,"seleph  : time=%s sat=%2d iode=%d\n",rtkcmn.time_str(time,3),sat,iode);

        sys=rtkcmn.satsys(sat,null);
        switch (sys) {
            case rtklib.SYS_GPS: tmax=rtklib.MAXDTOE+1.0    ; sel=eph_sel[0]; break;
            case rtklib.SYS_GAL: tmax=rtklib.MAXDTOE_GAL    ; sel=eph_sel[2]; break;
            case rtklib.SYS_QZS: tmax=rtklib.MAXDTOE_QZS+1.0; sel=eph_sel[3]; break;
            case rtklib.SYS_CMP: tmax=rtklib.MAXDTOE_CMP+1.0; sel=eph_sel[4]; break;
            default: tmax=rtklib.MAXDTOE+1.0; break;
        }
        tmin=tmax+1.0;
        ...
        if (iode>=0||j<0) {
            rtkcmn.trace(3,"no broadcast ephemeris: %s sat=%2d iode=%3d\n",rtkcmn.time_str(time,0),
                    sat,iode);
            return null;
        }
        return nav->eph+j;
    ...
    {
        double t,tmax=rtklib.MAXDTOE_GLO,tmin=tmax+1.0;
        int i,j=-1;
        ...
        if (iode>=0||j<0) {
            rtkcmn.trace(3,"no glonass ephemeris  : %s sat=%2d iode=%2d\n",rtkcmn.time_str(time,0),
                    sat,iode);
            return null;
        }
        return nav.geph+j;
    ...
    {
        double t,tmax=rtklib.MAXDTOE_SBS,tmin=tmax+1.0;
        int i,j=-1;

        rtkcmn.trace(4,"selseph : time=%s sat=%2d\n",rtkcmn.time_str(time,3),sat);

        for (i=0;i<nav.ns;i++) {
            if (nav.seph.get(i).sat!=sat) continue;
            if ((t=Math.abs(rtkcmn.timediff(nav.seph.get(i).t0,time)))>tmax) continue;
            if (t<=tmin) {j=i; tmin=t;} /* toe closest to time */
        }
        if (j<0) {
                      ...
                      double[] dts)
    {
        ...
        rtkcmn.trace(4,"ephclk  : time=%s sat=%2d\n",rtkcmn.time_str(time,3),sat);
    ...
    *svh=-1;
    ...
    final rtklib.sbssatp_t sbs = new rtklib.sbssatp_t();
        ...
        rtkcmn.trace(4,"satpos_sbas: time=%s sat=%2d\n",rtkcmn.time_str(time,3),sat);
    ...
    *svh=0;
        ...
        switch (ephopt) {
            ...
            case rtklib.EPHOPT_SBAS  : return satpos_sbas(time,teph,sat,nav,   rs,dts,var,svh);
    ...
    *svh=-1;
                        ...
                        int ephopt, double *rs, double *dts, double *var, int *svh)
    {
        ...
        for (i=0;i<n&&i<2*rtklib.MAXOBS;i++) {
            ...
            if (j>=rtklib.NFREQ) {
                rtkcmn.trace(2,"no pseudorange %s sat=%2d\n",rtkcmn.time_str(obs[i].time,3),obs[i].sat);
                continue;
            }
        ...
        for (i=0;i<n&&i<2*rtklib.MAXOBS;i++) {
            rtkcmn.trace(4,"%s sat=%2d rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.3f svh=%02X\n",
                    rtkcmn.time_str(time[i],6),obs[i].sat,rs[i*6],rs[1+i*6],rs[2+i*6],
     ...
     *-----------------------------------------------------------------------------*/
    public static void satseleph(int sys, int sel)
    {
        switch (sys) {
            case rtklib.SYS_GPS: eph_sel[0]=sel; break;
            case rtklib.SYS_GLO: eph_sel[1]=sel; break;
            case rtklib.SYS_GAL: eph_sel[2]=sel; break;
            case rtklib.SYS_QZS: eph_sel[3]=sel; break;
            case rtklib.SYS_CMP: eph_sel[4]=sel; break;
            case rtklib.SYS_SBS: eph_sel[5]=sel; break;
        }
    }
}
